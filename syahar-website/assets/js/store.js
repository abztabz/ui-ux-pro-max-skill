...
(function () {
  ...
  const Store = {
    ...
    /* ---- emergency alerts ----
       `contact` is the family's local emergency contact — alerted
       alongside the coordinator, and shown in the admin feed. */
    addAlert(level, family, text, contact) {
      const db = load();
      const now = new Date();
      db.alerts.unshift({
        id: 'a-' + Date.now(),
        at: now.toISOString().slice(0, 10) + ' ' + now.toTimeString().slice(0, 5),
        level, family, text, contact: contact || null, resolved: false
      });
      save(db);
    },
